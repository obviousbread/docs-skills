...
This file follows the [AGENTS.md](https://agents.md) open standard (Linux Foundation / Agentic AI Foundation). Claude Code, Codex, Cursor, Windsurf, Copilot, Aider, Devin, Amp read it natively. In this repo `CLAUDE.md` and `GEMINI.md` use the `@AGENTS.md` import directive so a single source of truth applies across harnesses.
...
- In proofreading output, never use `—` or `Ё/ё`; prefer `–` or `-` by context, format personal initials without spaces (`В.И. Фамилия`), and keep compact abbreviations such as `в т.ч.` free of internal spaces.
