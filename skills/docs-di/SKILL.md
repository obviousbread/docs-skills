...
Прочитать `structure.md`, `tier_rules.md`, `knowledge_base.md` и `~/.docs-plugin/di/approvers.md` — **до** уточняющих вопросов и поиска ПС. Если `~/.docs-plugin/di/approvers.md` отсутствует — попросить пользователя скопировать `references/approvers.md.example` и заполнить ФИО.
...
См. `profstandart-rules.md`. Если ПС не указан — WebSearch на `profstandart.rosmintrud.ru`, представить 1-3 кандидата пользователю. Если указан — валидировать (актуальность, применимость).
...
Цепочка СОГЛАСОВАНО: собирается по `tier_rules.md` (ключи ролей). ФИО согласующих — из `~/.docs-plugin/di/approvers.md`. Если стоит `{{...}}` или `[уточнить]` — выполнить WebSearch на сайте учреждения, обновить файл, использовать актуальное значение.
