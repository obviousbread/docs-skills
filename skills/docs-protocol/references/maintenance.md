...
`_load_staff` обязан принимать как конкретный `.xlsx`, так и каталог, созданный `docs-staff-list`, и находить заголовки среди первых 15 строк. Не добавляй зависимость от соседнего скилла.
